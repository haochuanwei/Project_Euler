...
class Heap(object):
    ...
    def __init__(self, L, minheap=True):
        ...
        if minheap:
            self.operators = {'up': operator.lt, 'down': operator.gt, 'cross': operator.le}
        else:
            self.operators = {'up': operator.gt, 'down': operator.lt, 'cross': operator.ge}
        self.heapify()
    ...
    def __bubbledown(self, i):
        ...
        # common case: if both children are availble, check both and swap with the one with the smaller value
            ...
            if self.operators['down'](self.values[i], self.values[l]) and self.operators['cross'](self.values[l], self.values[r]):
                self.values[i], self.values[l] = self.values[l], self.values[i]
                # recursive call to bubble further
                self.__bubbledown(l)
            # check whether node i should be swapped with its right child
            elif self.operators['down'](self.values[i], self.values[r]) and self.operators['cross'](self.values[r], self.values[l]):
